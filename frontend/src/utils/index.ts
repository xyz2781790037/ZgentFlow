...
const DEFAULT_VALID_TYPES = new Set(["pdf", "txt", "md", "docx", "doc", "pptx", "ppt", "epub", "mhtml", "jpg", "jpeg", "png", "csv", "xlsx", "xls"]);
 ...
 */
export function kbFileTypeVerification(
  file: any,
  silent = false,
  ...
  const effectiveMaxFileSizeMB = Math.min(
    MAX_FILE_SIZE_MB,
  ...
  if (file.size > effectiveMaxFileSizeMB * 1024 * 1024) {
    if (!silent) {
      MessagePlugin.error(i18n.global.t('error.fileSizeExceeded', { size: effectiveMaxFileSizeMB }));
    }
    return true;
  }
  return false;

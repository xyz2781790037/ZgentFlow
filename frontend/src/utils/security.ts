...
function providerSourceFromImageSrc(src: string): string | null {
  ...
  try {
    ...
    const isFileProxy = url.pathname === '/files';
    if (!isFileProxy) {
      return null;
    }
...
const protectedFileCacheState = (() => {
  ...
  const scope = window as typeof window & {
    __zealragProtectedFileCacheV1__?: ProtectedFileCacheState;
  };
  scope.__zealragProtectedFileCacheV1__ ||= fresh();
  return scope.__zealragProtectedFileCacheV1__;
})();
...
export async function hydrateProtectedFileImages(
  ...
  await Promise.all(Array.from(images).map(async (img) => {
    ...
    const isProxyRequest =
      requestURL.includes('file_path=') && requestURL.startsWith('/files?');
    if (!isProxyRequest) {

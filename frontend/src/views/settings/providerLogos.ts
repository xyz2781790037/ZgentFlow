...
//             低饱和品牌色调。
//
// 调用方传入 provider id 拿到 { mode, url }；找不到时返回 undefined，
// 卡片会回落到原有的首字母 monogram。

const colorModules = import.meta.glob('@/assets/img/providers/color/websearch/*.svg', {
  eager: true,
  query: '?url',
  import: 'default',
}) as Record<string, string>;

const monoModules = import.meta.glob('@/assets/img/providers/mono/websearch/*.svg', {
  eager: true,
  query: '?url',
  import: 'default',
}) as Record<string, string>;

export type ProviderCategory = 'websearch';

...
const props = defineProps<{
  ...
  canEdit: boolean;
	canRetryOwn?: (item: KnowledgeItem) => boolean;
...
const emit = defineEmits<{
  ...
  (e: 'action', action: 'trace' | 'reparse' | 'cancel-parse' | 'move' | 'delete', item: KnowledgeItem): void;
}>();
...
const progressFor = (item: KnowledgeItem) => {
  if (item.parse_status === 'completed' || item.parse_status === 'finalizing') return 5;
  return Math.max(1, Math.min(5, props.progressById?.[item.id] || 1));
};
const computeStatus = (item: KnowledgeItem): StatusInfo => {
  if (item.parse_status === 'pending' || item.parse_status === 'processing') {
    return { label: `${progressFor(item)}/5`, theme: 'primary', icon: 'loading', spin: true };
  }
  if (item.parse_status === 'finalizing') {
    return { label: '5/5', theme: 'primary', icon: 'loading', spin: true };
  }
  if (item.parse_status === 'failed') {
    return { label: `${progressFor(item)}/5`, theme: 'danger', icon: 'close-circle' };
  }
  if (item.parse_status === 'cancelled') {
    return { label: `${progressFor(item)}/5`, theme: 'warning', icon: 'close-circle' };
  }
  if (item.parse_status === 'draft') {
    return { label: t('knowledgeBase.statusDraft'), theme: 'warning' };
  }
  if (item.parse_status === 'completed') {
    return { label: '5/5', theme: 'success', icon: 'check-circle' };
  }
...
const onMoreVisible = (id: string, visible: boolean) => {
  ...
  if (visible) {
    for (const item of props.items) {
      if (item.id !== id) item.isMore = false;
    }
  }
};
...
const canRetry = (item: KnowledgeItem) => !!props.canRetryOwn?.(item);
const hasAnyActions = computed(() => props.canEdit || props.items.some(canRetry));

const handleAction = (action: 'trace' | 'reparse' | 'cancel-parse' | 'move' | 'delete', item: KnowledgeItem) => {
  moreOpen.value = null;
  item.isMore = false;
...
<template>
  <div class="doc-list-view" :class="{ 'is-loading': loading }">
    ...
    <div class="doc-list-header" :class="{ 'is-stuck': headerStuck }" role="row">
	  ...
	  <div class="cell cell-actions" role="columnheader" v-if="hasAnyActions"></div>
    </div>
    ...
    <div class="doc-list-body">
      ...
      >
		...
		<div class="cell cell-actions" v-if="canEdit || canRetry(item)" @click.stop>
          <t-popup
            v-model:visible="item.isMore"
          ...
          >
            ...
            <template #content>
              <div class="row-menu">
                <div
                  v-if="item.parse_status !== 'draft'"
                  class="row-menu-item"
                  @click.stop="handleAction('trace', item)"
                >
                  <t-icon class="icon" name="chart-bar" />
                  <span>{{ t('knowledgeStages.viewTrace') }}</span>
                </div>
				<div
				  v-if="canEdit && isParseInFlight(item)"
                  class="row-menu-item"
				...
				<t-popconfirm
				  v-else-if="canEdit || canRetry(item)"
                  theme="warning"
				...
				<t-popconfirm
				  v-if="canEdit && canCancelParse(item)"
                  theme="warning"
				...
				<div v-if="canEdit" class="row-menu-item" @click.stop="handleAction('move', item)">
...
@media (max-width: 1220px) {
  .doc-list-header,
  .doc-list-row {
    grid-template-columns: 42px minmax(220px, 1fr) 86px 92px 126px 44px;
    padding-inline: 12px;
  }
}
...
@media (max-width: 980px) {
  .doc-list-header,
  .doc-list-row {
    grid-template-columns: 40px minmax(200px, 1fr) 86px 92px 42px;
  }
...
@media (max-width: 760px) {
  ...
  .doc-list-header,
  .doc-list-row {
    grid-template-columns: 36px minmax(0, 1fr) 70px 36px;
    padding-inline: 8px;
  }
  .doc-list-header { height: 36px; border-radius: 0; }
  .doc-list-row { min-height: 68px; }
  .cell-time { display: none; }
  .cell { padding-inline: 5px; }
  .row-file-name { font-size: 13px; }
  .row-file-desc { max-width: 100%; }

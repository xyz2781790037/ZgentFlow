...
export function filterUploadFiles(
  ...
  for (const file of list) {
    ...
    if (UPLOAD_VIDEO_EXTENSIONS.includes(fileExt)) {
      unsupportedVideoCount++
      continue
    }

    if (kbFileTypeVerification(file, multiFile, dynamicTypes, options.maxFileSizeMB)) {
      skippedCount++
      continue
    }
  ...
  return { validFiles, skippedCount, unsupportedVideoCount, hiddenFileCount }
}

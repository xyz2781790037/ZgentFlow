  ...
  <div class="credential-resource">
    <div v-for="field in fields" :key="field.key" class="credential-row">
      <!--
        ...
        (for example api_key + a provider-specific app_secret) so a single-field card
        doesn't double up with the parent's outer .form-label. Single-field
        consumers (ModelEditorDialog API key and WebSearch provider api_key)
        keep showing the parent label only.
      -->

<template>
  <ZealModelWorkbench :visible="dialogVisible" :title="isEdit ? $t('model.editor.editTitle') : $t('model.editor.addTitle')"
      ...
      <template v-if="formData.source === 'remote'">
        <section class="setting-drawer__section">
          ...
          <div class="form-item">
            ...
            <t-select v-model="formData.provider" :loading="loadingProviders"
          ...
          <div class="form-item credential-source-field">
            <div class="credential-label-row">
              <label class="form-label">API 凭证</label>
              <t-button variant="text" size="small" theme="primary" @click="goToAPIConfigSettings">
                <template #icon><t-icon name="setting" /></template>
                管理 API
              </t-button>
            </div>

            <div class="credential-source" role="radiogroup" aria-label="API 凭证来源">
              <button
                type="button"
                :class="{ active: formData.credentialMode === 'saved' }"
                role="radio"
                :aria-checked="formData.credentialMode === 'saved'"
                @click="selectCredentialMode('saved')"
              >
                <t-icon name="folder-open" />已有 API
              </button>
              <button
                type="button"
                :class="{ active: formData.credentialMode === 'manual' }"
                role="radio"
                :aria-checked="formData.credentialMode === 'manual'"
                @click="selectCredentialMode('manual')"
              >
                <t-icon name="edit" />手动填写
              </button>
            </div>

            <template v-if="formData.credentialMode === 'saved'">
              <t-select
                v-model="formData.apiConfigId"
                :loading="loadingAPIConfigs"
                placeholder="选择当前服务商下的 API 配置"
                clearable
              >
                <t-option
                  v-for="config in providerAPIConfigs"
                  :key="config.id"
                  :value="config.id"
                  :label="config.name"
                >
                  <div class="api-config-option">
                    <span>{{ config.name }}</span>
                    <small>{{ config.model_count }} 个模型引用</small>
                  </div>
                </t-option>
              </t-select>
              <p v-if="!loadingAPIConfigs && providerAPIConfigs.length === 0" class="credential-warning">
                当前服务商还没有 API 配置，请先添加或改为手动填写。
              </p>
              <p v-else-if="selectedAPIConfigUnavailable" class="credential-warning">
                原 API 配置已删除，请重新选择。
              </p>
              <p v-else class="form-desc">模型会持续引用该配置，后续更新 Key 无需逐个修改模型。</p>
            </template>

            <template v-else>
              <CredentialResource
                v-if="isEdit && props.modelData?.id && !formData.saveApiConfig"
                :api="credentialApi"
              ...
              <t-checkbox v-model="formData.saveApiConfig">保存为可复用 API 配置</t-checkbox>
              <t-input
                v-if="formData.saveApiConfig"
                v-model="formData.apiConfigName"
                maxlength="100"
                placeholder="配置名称，例如：生产环境"
              />
              <p class="form-desc">
                {{ formData.saveApiConfig ? '保存模型时会创建配置，并让当前模型持续引用。' : '仅用于当前模型，不会出现在 API 管理中。' }}
              </p>
            </template>
...
import { listModelAPIConfigs, type ModelAPIConfig } from '@/api/model-api-config'
...
import ZealModelWorkbench from '@/components/settings/ZealModelWorkbench.vue'
...
type EditorModelType = 'chat' | 'embedding' | 'rerank' | 'vllm'
...
const apiConfigs = ref<ModelAPIConfig[]>([])
const loadingAPIConfigs = ref(false)
const apiConfigsLoaded = ref(false)
const enabledProviderIds = ['siliconflow', 'deepseek', 'hunyuan', 'generic']
...
const providerOptions = computed(() => {
  ...
  if (apiProviderOptions.value.length > 0) {
    return apiProviderOptions.value
      .filter(p => enabledProviderIds.includes(p.value))
      .sort((a, b) => enabledProviderIds.indexOf(a.value) - enabledProviderIds.indexOf(b.value))
      .map(p => ({
        ...p,
...
const providerAPIConfigs = computed(() =>
  apiConfigs.value.filter(config => config.provider === formData.value.provider),
)

const selectedAPIConfigUnavailable = computed(() => {
  const id = formData.value.apiConfigId
  return apiConfigsLoaded.value
    && !!id
    && !providerAPIConfigs.value.some(config => config.id === id)
})

const loadAPIConfigs = async () => {
  loadingAPIConfigs.value = true
  try {
    apiConfigs.value = await listModelAPIConfigs()
    apiConfigsLoaded.value = true
  } catch (error: any) {
    apiConfigs.value = []
    MessagePlugin.error(error?.message || 'API 配置加载失败')
  } finally {
    loadingAPIConfigs.value = false
  }
}
...
const goToAPIConfigSettings = () => {
  emit('update:visible', false)
  uiStore.openSettings('api-configs')
}

const selectCredentialMode = (mode: 'saved' | 'manual') => {
  formData.value.credentialMode = mode
  remoteChecked.value = false
  remoteAvailable.value = false
  remoteMessage.value = ''
  if (mode === 'manual') {
    formData.value.apiConfigId = ''
  } else {
    formData.value.saveApiConfig = false
    formData.value.apiConfigName = ''
  }
}
    ...
    try {
      if (props.modelData) {
        ...
        formData.value = {
          ...props.modelData,
          apiKey: '',
          credentialMode: props.modelData.apiConfigId ? 'saved' : 'manual',
          saveApiConfig: false,
          apiConfigName: '',
        }
...
const checkRemoteAPI = async () => {
  ...
  if (formData.value.credentialMode === 'saved' && !formData.value.apiConfigId) {
  ...
  try {
    ...
    const idPayload = isEdit.value && props.modelData?.id
      ...
      : {}
    const apiConfigPayload = {
      apiConfigId: formData.value.credentialMode === 'saved'
        ? (formData.value.apiConfigId || '')
        : '',
    }
...
const handleConfirm = async () => {
  try {
    ...
    if (formData.value.source === 'remote') {
      ...
      if (formData.value.credentialMode === 'manual' && formData.value.saveApiConfig) {
        if (!formData.value.apiConfigName?.trim() || !formData.value.apiKey?.trim()) {
...
.credential-source {
  ...
  button {
    ...
    &.active {
      color: #0d58bf;
      background: #fff;
      box-shadow: 0 1px 3px rgba(32, 55, 84, 0.12);
      font-weight: 600;
    }
  }
}
...
.api-config-option {
  ...
  small { flex: 0 0 auto; color: #8591a2; font-size: 11px; }
}
...
// Destructive-action checkbox for "Remove this credential".
.clear-credential {
  display: inline-flex;
  margin-top: 8px;

...
-- Controls which indexing pipelines are active (vector, keyword, wiki).
-- Backfill: existing rows get vector+keyword=true (legacy default behavior);
-- wiki_enabled stays false until explicitly enabled.
-- ---------------------------------------------------------------------------
ALTER TABLE knowledge_bases ADD COLUMN IF NOT EXISTS indexing_strategy JSONB;

COMMENT ON COLUMN knowledge_bases.indexing_strategy IS 'Indexing pipelines strategy: {"vector_enabled": bool, "keyword_enabled": bool, "wiki_enabled": bool}';

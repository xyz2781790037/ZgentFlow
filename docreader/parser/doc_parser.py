...
class CommandExecutor:
    """Runs local parser commands with outbound network access blocked."""
    ...
    def execute(self, cmd: List[str]) -> tuple:
        """Execute a parser command with the configured proxy environment.
        ...
        """
        return self._execute_with_proxy(cmd)
...
class DocParser(Docx2Parser):
    ...
    def _parse_with_antiword(self, temp_file_path: str) -> Document:
        ...
        logger.info("Executing antiword with outbound network blocked")

        stdout, stderr, returncode = self.command_executor.execute(cmd)
    ...
    def _try_convert_doc_to_docx(self, doc_path: str) -> Optional[bytes]:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            with TempDirContext() as temp_dir, TempDirContext() as profile_dir:
                ...
                logger.info(
                    f"Running conversion command (attempt {attempt}/{max_attempts}): "
                    f"{' '.join(cmd)}"
                )

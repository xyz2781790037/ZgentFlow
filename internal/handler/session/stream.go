...
// handleStreamEventsForSSE polls the quick-answer stream and pushes events to
// SSE, allowing graceful handling of disconnections.
// waitForTitle: if true, wait for title event after completion (for new sessions without title)
func (h *Handler) handleStreamEventsForSSE(
	ctx context.Context,
	c *gin.Context,

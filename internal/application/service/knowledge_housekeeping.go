...
// ConfigureTrashServices enables the daily recycle-bin purge.
func (h *HousekeepingService) ConfigureTrashServices(
	kbService interfaces.KnowledgeBaseService,
	knowledgeService interfaces.KnowledgeService,
) {
...
func (h *HousekeepingService) Start(ctx context.Context) error {
	...
	if !housekeepingEnabled() {
		logger.Infof(ctx, "[Housekeeping] disabled via ZEALRAG_HOUSEKEEPING_ENABLED=false")
		return nil
	}
	...
	if _, err := h.cron.AddFunc("0 0 3 * * *", func() {
		h.purgeExpiredTrash(context.Background())
	}); err != nil {
		return err
	}
	h.cron.Start()
...
func (h *HousekeepingService) purgeExpiredTrash(ctx context.Context) {
	if h.kbService == nil || h.knowledgeService == nil {
		return
	}
	...
	var documents []trashRow
	if err := h.db.WithContext(ctx).Unscoped().Model(&types.Knowledge{}).
		Select("id, tenant_id").
		Where("deleted_at IS NOT NULL AND deleted_at <= ?", cutoff).
		Find(&documents).Error; err != nil {
		logger.Warnf(ctx, "[Housekeeping] failed to scan expired document trash: %v", err)
	} else {
		for _, row := range documents {
			if err := h.knowledgeService.PurgeTrashedKnowledgeForTenant(ctx, row.TenantID, row.ID); err != nil {
				logger.Warnf(ctx, "[Housekeeping] failed to purge document %s: %v", row.ID, err)
			}
		}
	}

	var knowledgeBases []trashRow
	if err := h.db.WithContext(ctx).Unscoped().Model(&types.KnowledgeBase{}).
		Select("id, tenant_id").
		Where("deleted_at IS NOT NULL AND deleted_at <= ? AND is_temporary = ?", cutoff, false).
		Find(&knowledgeBases).Error; err != nil {
		logger.Warnf(ctx, "[Housekeeping] failed to scan expired knowledge-base trash: %v", err)
	} else {
		for _, row := range knowledgeBases {
			if err := h.kbService.PurgeTrashedKnowledgeBaseForTenant(ctx, row.TenantID, row.ID); err != nil {
				logger.Warnf(ctx, "[Housekeeping] failed to purge knowledge base %s: %v", row.ID, err)
			}
		}
	}
}
...
func (h *HousekeepingService) runSweep(ctx context.Context) {
	...
	//
	// Knowledge rows with no spans (for example, tasks created before
	// tracing was introduced) fall back to the simple
	// updated_at check — they have no heartbeat to consult.
	// Include 'finalizing' alongside 'processing': finalizing rows still
	// consume LLM compute via enrichment subtasks (summary/question/Wiki),
	...
	// question / Wiki) are merely backlogged behind a busy queue
	...
	if queueSkipped > 0 {
		...
		// mean the queue is the bottleneck (raise ZEALRAG_ASYNQ_CONCURRENCY
...
// with no span rows at all also pass through (they predate tracing; the
// simple updated_at check already proved
...
func (h *HousekeepingService) filterByLastSpanActivity(ctx context.Context, candidates []types.Knowledge, cutoff time.Time) []types.Knowledge {
	...
	// aggregate datetime values into time.Time on its own. Since we only
	// compare against a cutoff, parse the PostgreSQL timestamp explicitly.
...
func housekeepingEnabled() bool {
	...
	v := strings.TrimSpace(os.Getenv("ZEALRAG_HOUSEKEEPING_ENABLED"))
	if v == "" {
		return true
	}

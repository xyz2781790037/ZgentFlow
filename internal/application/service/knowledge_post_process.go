...
func (s *KnowledgePostProcessService) Handle(ctx context.Context, task *asynq.Task) error {
	...
	// question / wiki child tasks for an aborted knowledge. We
	...
	// Only text chunks participate in downstream enrichment.
	...
	case expectedSubtasks == 0:
		...
		// users without summary/question/wiki see 'completed' immediately.
	...
	// Summary and question generation each own a counted slot that only their own
	// task drains; a slot whose task was never enqueued has no owner
	...
	if enteredFinalizing {
		plannedOwned := questionBatchCount
		if willSpawnSummary {
			plannedOwned++
		}
		actualOwned := enqueuedQuestionCount
		if enqueuedSummary {
			actualOwned++
		}
	...
	// downstream stages (summary/question/wiki) record their
...
func (s *KnowledgePostProcessService) enqueueSummaryGenerationTask(ctx context.Context, payload types.KnowledgePostProcessPayload, attempt int) bool {
	...
	if _, err := s.taskEnqueuer.Enqueue(task, asynq.Queue("low"), asynq.MaxRetry(3)); err != nil {
...
func (s *KnowledgePostProcessService) enqueueQuestionGenerationTasks(
	...
	for start := 0; start < total; start += questionGenChunkBatchSize {
		...
		if _, err := s.taskEnqueuer.Enqueue(task, asynq.Queue(types.QueueQuestion), asynq.MaxRetry(3)); err != nil {

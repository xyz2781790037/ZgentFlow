...
// externalURL is the externally-reachable base URL (e.g. "https://zealrag.example.com");
...
func (s *localFileService) GetFileURL(ctx context.Context, filePath string) (string, error) {
	...
	if s.externalURL != "" {
		// Tenant ID is encoded in every local storage path and bound into
		// the signature so a URL cannot be reused for another workspace.

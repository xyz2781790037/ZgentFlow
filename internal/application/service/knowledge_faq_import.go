...
func (s *knowledgeService) generateFailedEntriesCSV(ctx context.Context,
	...
	// 写入表头
	buf.WriteString("错误原因,问题(必填),相似问题(选填-多个用##分隔),反例问题(选填-多个用##分隔),机器人回答(必填-多个用##分隔),是否全部回复(选填-默认FALSE),是否停用(选填-默认FALSE)\n")
	...
	for _, entry := range failedEntries {
		...
		buf.WriteString(fmt.Sprintf("%s,%s,%s,%s,%s,%s,%s\n",
			reason, standardQ, similarQs, negativeQs, answers, answerAll, isDisabled))
	}

...
func (p *PluginQueryUnderstand) loadHistory(ctx context.Context, chatManage *types.ChatManage) []*types.History {
	...
	// explicitly by applyAgentOverridesToChatManage when the answer mode has
...
func (p *PluginQueryUnderstand) selectModel(ctx context.Context, chatManage *types.ChatManage, hasImages bool) (chat.Chat, bool) {
	if hasImages {
		if chatManage.ChatModelSupportsVision {
			m, err := p.modelService.GetChatModel(modelContext(ctx, chatManage), chatManage.ChatModelID)
			if err == nil {
				return m, true
			}
		...
		if chatManage.VLMModelID != "" {
			m, err := p.modelService.GetChatModel(modelContext(ctx, chatManage), chatManage.VLMModelID)
			if err == nil {
				return m, true
			}
	...
	m, err := p.modelService.GetChatModel(modelContext(ctx, chatManage), textModelID)
	if err != nil {
		...
		if chatManage.QueryUnderstandModelID != "" && textModelID != chatManage.ChatModelID {
			...
			if fallback, fbErr := p.modelService.GetChatModel(modelContext(ctx, chatManage), chatManage.ChatModelID); fbErr == nil {

...
import (
	...
	"github.com/xyz2781790037/ZealRAG/internal/searchutil"
...
func (p *PluginSearch) searchByTargets(
	...
	for _, t := range chatManage.SearchTargets {
		key := fmt.Sprintf("%d|%s", t.TenantID, modelKeyMap[t.KnowledgeBaseID])
	...
	for modelKey, targets := range groups {
		...
		go func(modelKey string, targets []*types.SearchTarget) {
			defer wg.Done()
			targetCtx := types.WithTenantID(ctx, targets[0].TenantID)

			// Compute embedding once for this model group.
			var queryEmbedding []float32
			if modelKey != "" {
				emb, err := p.knowledgeBaseService.GetQueryEmbedding(targetCtx, targets[0].KnowledgeBaseID, queryText)
				if err != nil {
			...
			if len(fullKBIDs) > 0 {
				...
				go func() {
					...
					res, err := p.knowledgeBaseService.HybridSearch(targetCtx, fullKBIDs[0], params)
					if err != nil {
			...
			for _, target := range knowledgeTargets {
				...
				go func(t *types.SearchTarget) {
					defer innerWg.Done()
					p.searchSingleTarget(types.WithTenantID(ctx, t.TenantID), chatManage, t, queryText, queryEmbedding, &mu, &results)
				}(target)
			}
...
func (p *PluginSearch) searchSingleTarget(
	...
	if t.Type == types.SearchTargetTypeKnowledge {
		directResults, skippedIDs := p.tryDirectChunkLoading(ctx, t.TenantID, t.KnowledgeIDs)

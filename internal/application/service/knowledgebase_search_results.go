...
func (s *knowledgeBaseService) processSearchResults(ctx context.Context,
	...
	knowledgeMap, err := s.fetchKnowledgeDataBatch(ctx, tenantID, index.knowledgeIDs)
	if err != nil {
		return nil, err
	}
	...
	allChunks, err := s.listChunksByIDBatch(ctx, tenantID, index.chunkIDs)
	if err != nil {
	...
	if !skipEnrichment {
		...
		if len(additionalChunkIDs) > 0 {
			...
			additionalChunks, err := s.listChunksByIDBatch(ctx, tenantID, additionalChunkIDs)
			if err != nil {
				...
				if s.hasImageChunks(allChunks) {
					...
					if len(parentIDs) > 0 {
						...
						parentChunks, err := s.listChunksByIDBatch(ctx, tenantID, parentIDs)
						if err != nil {

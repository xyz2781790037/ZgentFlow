...
func (g *pgRepository) KeywordsRetrieve(ctx context.Context,
	...
	// Use PostgreSQL pg_trgm for fuzzy matching with an exact substring fallback.
	conds = append(conds, clause.Expr{
		SQL:  "(content % ? OR content ILIKE '%' || ? || '%')",
		Vars: []interface{}{params.Query, params.Query},
	})
	...
	err := g.db.WithContext(ctx).Clauses(conds...).Debug().
		Select(
			"GREATEST(similarity(content, ?), CASE WHEN content ILIKE '%' || ? || '%' THEN 1.0 ELSE 0.0 END) AS score, "+
...
func (g *pgRepository) VectorRetrieve(ctx context.Context,
	...
	err := g.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		// knowledge_id / is_enabled) yields enough rows. Without it,

...
func (r *knowledgeRepository) GetTrashedKnowledge(
...
func (r *knowledgeRepository) ListTrashedKnowledge(
...
func (r *knowledgeRepository) RestoreKnowledge(
	ctx context.Context, tenantID uint64, id string,
) error {
	result := r.db.WithContext(ctx).Unscoped().Model(&types.Knowledge{}).
		Where("tenant_id = ? AND id = ? AND deleted_at IS NOT NULL", tenantID, id).
...
func (r *knowledgeRepository) PurgeKnowledge(
	ctx context.Context, tenantID uint64, id string,
) error {
...
func (r *knowledgeRepository) PurgeKnowledgeList(

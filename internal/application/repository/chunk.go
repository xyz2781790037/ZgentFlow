...
func (r *chunkRepository) UpdateChunks(ctx context.Context, chunks []*types.Chunk) error {
	...
	// Combine args in correct order: content, is_enabled, flags, status, then IN clause
	var args []interface{}
...
// ListAllFAQChunksForExport lists all FAQ chunks for export with metadata, status, and flags.
// Uses batch query to handle large datasets.
func (r *chunkRepository) ListAllFAQChunksForExport(
	...
	for {
		var batchChunks []*types.Chunk
		if err := r.db.WithContext(ctx).
			Select("id, metadata, is_enabled, flags").

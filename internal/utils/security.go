...
func isSSRFSafeURL(rawURL string) (bool, string) {
	...
	// Reject common internal-service ports before DNS resolution. Besides
	// failing fast, this keeps the rule deterministic when a test or proxy
	// environment rewrites public DNS to a reserved address.
	port := parsed.Port()
	if port != "" {
		blockedPorts := map[string]bool{
			"22": true, "23": true, "25": true, "445": true, "3389": true,
			"5432": true, "3306": true, "6379": true, "27017": true,
			"9200": true, "2379": true, "2380": true, "8500": true, "4001": true,
		}

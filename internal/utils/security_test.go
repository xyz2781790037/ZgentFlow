...
func TestSSRFSafeURL_AllowPublicDomain(t *testing.T) {
	...
	if !ok {
		// This path depends on runtime DNS/network. Some sandbox/proxy
		// environments deliberately map public domains to a reserved range.
		if strings.Contains(reason, "DNS resolution failed") ||
			strings.Contains(reason, "resolves to restricted IP") {
			t.Skipf("skip due to test-environment DNS mapping: %s", reason)
		}

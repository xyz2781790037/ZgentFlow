...
func (c *MinerUReader) Read(ctx context.Context, req *types.ReadRequest) (*types.ReadResult, error) {
	...
	mdContent, _, err := c.callFileParse(ctx, content)
	if err != nil {
	...
	logger.Infof(context.Background(), "[MinerU] Parsed successfully, markdown=%d chars", len(mdContent))
...
var imgHTMLRelativeSrc = regexp.MustCompile(`(?i)<img\b[^>]*\bsrc=["']([^"']+)["'][^>]*>`)
